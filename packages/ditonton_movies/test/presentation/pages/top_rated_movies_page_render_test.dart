...
void main() {
  testWidgets('Top rated page renders MovieCard when loaded with data', (tester) async {
    final repo = _Repo(const [Movie(
      adult: false, backdropPath: 'b', genreIds: [], id: 2,
      originalTitle: 'OT', overview: 'OV', popularity: 0,
      posterPath: '/p', releaseDate: '2020-01-01', title: 'TR', video: false,
      voteAverage: 0, voteCount: 0,
    )]);
    await tester.pumpWidget(ChangeNotifierProvider<TopRatedMoviesNotifier>(
      create: (_) => TopRatedMoviesNotifier(getTopRatedMovies: GetTopRatedMovies(repo)),
